...
class PlanCreator:
    ...
    def set_fields(self, count: models.Count):
        ...
        self.set_text_item(
            "f_07", "{} + {} m".format(section.start_pr, round(section.start_dist, 3))
        )
        self.set_text_item(
            "f_08", "{} + {} m".format(section.end_pr, round(section.end_dist, 3))
        )
        self.set_text_item("f_09", section.place_name)
        self.set_text_item("f_10", count.start_process_date.strftime("%d.%m.%Y (%A)"))
        self.set_text_item("f_11", count.end_process_date.strftime("%d.%m.%Y (%A)"))
        self.set_text_item("f_14", "")
        self.set_text_item("f_15", "")

        direction_desc = ""
        if models.Lane.objects.filter(
            id_installation__count=count, direction=1
        ).first():
            direction_desc = (
                models.Lane.objects.filter(id_installation__count=count, direction=1)
                .first()
                .direction_desc
            )
        self.set_text_item("f_30", direction_desc)

        direction_desc = ""
        if models.Lane.objects.filter(
            id_installation__count=count, direction=2
        ).first():
            direction_desc = (
                models.Lane.objects.filter(id_installation__count=count, direction=2)
                .first()
                .direction_desc
            )
        self.set_text_item("f_31", direction_desc)

        # Page 2
        self.set_text_item("f_17", "Campagne de comptage")
        self.set_text_item(
            "f_18", "Pose {}".format(count.start_put_date.strftime("%A %d.%m.%Y"))
        )
        self.set_text_item(
            "f_19", "Dépose {}".format(count.end_put_date.strftime("%A %d.%m.%Y"))
        )
        ...
        self.set_picture_item(
            "logo", os.path.join(current_dir, os.pardir, "images", "logo_ne.png")
        )
    ...
    def set_picture_item(self, name, file_name):
        ...
        picture_path = os.path.join(self.settings.value("picture_directory"), file_name)
    ...
    def create_layout_from_template(template_filename):
        ...
        with open(os.path.join("data", "general", template_filename)) as template_file:
