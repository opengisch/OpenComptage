...
class ConfigCreatorCmd(ConfigCreator):
    ...
    def set_section_commands(self):
        ...
        self.set_command(
            "SITE", self.layers.get_installation_name_of_count(self.count_id)
        )
        self.set_command("LOCATION", sections[0].attribute("name"))
        self.set_command(
            "FILENAME", self.layers.get_installation_name_of_count(self.count_id)
        )
        self.set_command("CLASS", self.layers.get_class_name_of_count(self.count_id))
