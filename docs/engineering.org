...
*** Tables
    | English name   | French name      | Meaning of a record |
    |----------------+------------------+---------------------|
...
**** lane
     ...
     from the existing BASE_TJM_OK MapInfo table.
*** Old - to be removed after review
**** measurement
...
**** Buttons
     - Connect to the DB
       - Check if all the needed layers are loaded into the current project
         otherwise it load them. If a layer with the same name is loaded but
         from a different source, display a message.
     - Create a new campaign
     - Show the existing measure campaigns (as a button or implicit with the
       connect DB?)
     - Select a measure campaign
       - Edit a measure campaign
       - Export device configuration file
       - Create a measure plan
       - Import measure data
       - Create report
     - Import special periods
     - Settings
**** Flow
     - At the beginning all the buttons except "connect to DB" and "settings"
       are disabled. Once we click on connect to the DB, if the connection in
       ok, the buttons became enabled.
       - [X] Create all the buttons
       - [X] Set the icons of the buttons
       - [ ] Prepare the settings dialog
       - [X] Prepare the connection part
       - [X] Make the button enable/disable depending on the connection
       - [ ] functional tests 
** TODOs
*** TODO add "Create a comptage" Use case
*** TODO change campaign with "comptage" or "count"
*** TODO commit changes
*** TODO update data model with "comptage agregé"
*** TODO prepare chart examples
    - vehicles per hours (with one bar per day)
    - total of vehicles per category
    - vehicles per speed (10km steps)
*** TODO add req. for allowing copying data from a comptage to another one
*** TODO Define meeting with Pierre for data import issues
*** TODO complete filter
*** TODO complete import data with progression bar etc.
*** TODO integrate plotly into comptages
